; CK401 Final Year Project - James Whyte
; This project is based on FlashForth by Mikael Nordman
; Flashforth is distributed under the GNU general public license v3,
; and can be found here: <https://github.com/oh2aun/flashforth/tree/master>
;**********************************************************************
...
FETCHOP_:
    ...
    call FETCH
    ret
; Fetch ulink (link to next task)
FETCHUL_:
    call ULINK_
    call FETCH
    ret
; Set ulink (link to next task)
STOREUL_:
    call ULINK_
    call STORE
    ret
STOREOP_:
    rcall FETCHOP_ ; Get XT of operator task
    rcall STOREUP_ ; Store opXT in User Program
    ret
 
; Access user variables of other tasks (task-addr var-addr -- addr)
HIS_L:
    .byte   NFA|3
    .ascii  "his"
    .align  1
...
DEFTASKSKIP:
    ...
    push    r27	    ; Then push FLASH HERE to data stack (not return stack)
    push    r26
    ; Adjust for hold pointer
    call    ONEPLUS ;1+
    ; For looping 4 times
    ldi	    r26, 4
    ldi	    r27, 0
...
DEFTASKLOOP:
    call    SWOP     ; (...param addr count -- ) -> (...count addr param)
    m_dup	    ; dup
    call    ROM_    ; flash
    call    COMMA   ; ,
    call    FRAM    ; ram
    call    ALLOT   ; allot
    ; Loop Logic
    call    ONEMINUS; decrement counter (originally 4)
    rcall   POPF_
    cpi	    r26, 0  ; if r26==0, end
    breq    DEFTASKEND
    rcall   PUSHF_  ; Else push counter,
    rcall   DEFTASKLOOP ;  and restart loop
    ret
DEFTASKEND:
    ret
fdw DEFTASK_L
...
; ( taskloop-addr task-addr -- )
TINIT_L:
    .byte   NFA|5
    .ascii  "tinit"
    .align  1
TINIT:
    ; Fetch value at task-addr and increment pointer
    call    FETCHPP
    ; Store fetched val in UP
    call    STOREUP_
    ; Fetch value in UL - indicates if task is running
    call    FETCHUL_
    rcall   POPF_
    ; If UL is non-zero, task is already running, exit
    cpi	    r26, 0
    brne    TINITRLY
    cpi	    r27, 0
    brne    TINITRLY
...
TINITELSE:
    ...
    call    DUP		; Duplicate TOS (taskloop-addr)
    call    TWOMINUS	; 2- decrement by 2
    call    TASK	; task pointer
    call    STORE	; ! Store decremented taskloop-addr value in task
    ; Compute base return pointer r0 as uarea + addsize + rsize
    call    FETCHPP	; @+
    call    SWOP	; swap
    call    FETCHPP	; @+
    call    ROT		; rot
    ...
    call    R0_		;r0
    call    STORE	;!
    ; Compute initial stack pointer s0 as r0 + ssize + 1
    call    FETCH	;@
    call    R0_		;r0
    call    FETCH	;@
    call    PLUS	;+
    ...
    call    S0		;s0
    call    STORE	;!
    ; Store task-loop address to return stack
    call    R0_		;r0
    call    FETCH	;@
    call    X_TO_R	;x>r
    ; Push SP to Return stack
    call    ONEMINUS	;1-
    call    DUP		;dup
    call    S0		;s0
    call    FETCH	;@
    ...
    ldi	    r26, 5
    ldi	    r27, 0
    rcall   PUSHF_	; Push 5 to TOS
    call    MINUS	;-
    call    RSAVE_	;rsave
    call    STORE	;!
    ; Set tiu to s0 (data stack pointer)
    call    S0		;s0
    call    FETCH	;@
    call    TIU		;tiu
    call    STORE	;!
    ...
    ldi	    r27, 0
    call    DUP
    rcall   PUSHF_	;Push 0 to TOS
    rcall   STOREUL_	;Clear UL
    ; Clear status and control flags
    call    TASK
    call    TWOPLUS	;2+
    call    STORE	;!
    ; Change base back to decimal
    call    DECIMAL
;    ldi	    r26, 26 ; Debug stuff
;    clr	    r27
;    rcall   PUSHF_
    jmp	    TINITEX
fdw TINIT_L
...
TEND:
    ...
    rcall   FETCHUL_
    rcall   POPF_
    ; If UL is zero, task is NOT running, exit
    ldi	    r16, 0
    cp	    r16, r26
    brne    TENDELSE
    cp	    r16, r27
...
TENDEX:
    ...
    ret
TENDELSE:
    rcall   FETCHUP_	    ; Current UP is the target task - get it on TOS
    rcall   STOREOP_	    ; Switch to op task
    call    DUP		    ; Duplicate target task addr
    ; Maintain a copy of the target uarea in r16 and r17
    rcall   POPF_
    mov	    r16, r26 
    mov	    r17, r27
    rjmp    TENDLOOP
TENDLOOP:
    ...
    rcall   POPF_
    cp	    r16, r26
    brne    RSTENDLOOP ;restart loop
    cp	    r17, r27
    brne    RSTENDLOOP
    rjmp    TENDCONT	; If the correct task is found, break the loop
; This extra step is needed to carry out some operations b4 looping
RSTENDLOOP: 
    rcall   FETCHUL_
    rcall   STOREUP_
    rjmp    TENDLOOP
TENDCONT:
    rcall   FETCHUP_	; Get previous user area
    call    SWOP
    rcall   STOREUP_	; Store uarea at user pointer
    rcall   FETCHUL_	; Fetch next-uarea
    clr	    r26
    clr	    r27
    rcall   PUSHF_	; Push 0 to stack
    rcall   STOREUL_	; Ulink of an ended task is 0
    call    SWOP
    rcall   STOREUP_	; Store prev-uarea in user pointer
    rcall   STOREUL_	; Store next-uarea in user-link
    rjmp TENDEX
fdw TEND_L
...
TSINGLE:
    ...
    mov	    r17, r27
    rcall   POPF_
    ; Compare operator task link and user area to determine if any other tasks exist
    cp	    r16, r26
    brne    TSINGLEM	;TSingleMain
    cp	    r17, r27
    brne    TSINGLEM
    ret			; Exit if no other tasks
TSINGLEM:
    ; Link operator to itself
    rcall   FETCHUL_
    rcall   FETCHOP_
    rcall   STOREUL_
    ; Move to next user area by storing current ul val in up pointer
    rcall   STOREUP_
    rjmp    TSINGLELOOP
TSINGLELOOP:
    rcall   FETCHUL_
    ...
    mov	    r17, r27
    rcall   POPF_
    ; Check if the current user area's ul pointer does not point to the operator task (i.e., more tasks remain).
    cp	    r16, r26
    brne    TSINGLEOP	;TSingleOperation
    cp	    r17, r27
    brne    TSINGLEOP
    rjmp    TSINGLEND	;UL == OP, move to exit
TSINGLEOP:
    rcall   FETCHUL_
    clr	    r26
    clr	    r27
    rcall   PUSHF_	; Push 0 to stack
    rcall   STOREUL_	; Write 0 to ulink
    rcall   STOREUP_	; Move to next user area
    rjmp    TSINGLELOOP	; Repeat
TSINGLEND:
    clr	    r26
    clr	    r27
    rcall   PUSHF_	; Push 0 to stack
    rcall   STOREUL_	; Write 0 to final ulink
    rcall   STOREOP_	; Switch to operator task
    ret
fdw TSINGLE_L
...
TASKS_L:
    ...
    .align  1
TASKS:
    ; Fetch Running Task and switch to operator task
    rcall   FETCHUP_
    rcall   STOREOP_
    rjmp    TASKSLOOP
TASKSLOOP:
    rcall   FETCHUP_	; Fetch current user area
    call    TASK	; Push task definition table addr to stack
    call    FETCH	; Fetch val at table address
    clr	    r27
    ldi	    r26, 12	; Push 12 to TOS
    rcall   PUSHF_
    call    MINUS	; Subtract 12
    rcall   STOREOP_	; Switch to operator task
    call    CFATONFA	; Convert CFA to NFA
    call    DOTID	; Print task name corresponding to NFA
    call    SPACE_	; Emit space char
    rcall   STOREUP_	; Move to next userarea
    rcall   FETCHUL_
    rcall   FETCHOP_
    call    MINUS	; Subtract UL from OP
    rcall   POPF_	
    cpi	    r26, 0
    brne    TASKSRELOOP
    cpi	    r27, 0
    ...
    rjmp    TASKSEND
TASKSRELOOP:
    ; Fetch next user area with ul@ and update user pointer, then repeat
    rcall   FETCHUL_	
    rcall   STOREUP_
    rjmp    TASKSLOOP
TASKSEND:
...
; Define and initialize a task with parameters 0 30 30 10 (15 Item stack, 10 memory bytes)
;	(taskloop-addr -- )
MEDTASK_L:
    .byte   NFA|7
    .ascii  "medtsk:"
...
; Define and initialize a task with parameters 0 60 60 20 (30 Item stack, 20 memory bytes)
;	(taskloop-addr -- )
BIGTASK_L:
    .byte   NFA|7
    .ascii  "bigtsk:"
...
BIGTASK:
    ...
    rcall   PUSHF_  ; push 0
    ldi	    r26, 60
    rcall   PUSHF_  ; push 60
    rcall   PUSHF_  ; push 60
    ldi	    r26, 20
    rcall   PUSHF_  ; push 20 
    rjmp    DEFINRUN
fdw BIGTASK_L
    
; (taskloop-addr tibsize stacksize rsize addsize -- )
DEFINRUN_L:
    .byte   NFA|9
    .ascii  "definrun:"
    .align  1
...
TICK_L:
        ...
        .align  1
TICK:
        rcall   BL  ; Create an ASCII space character in mem
        rcall   WORD ; Gets XT of word from
        rcall   FIND
        jmp     QABORTQ
...
VER:
        call    XSQUOTE
        .byte     32
        .ascii    "Ardwin OS: Based on FlashForth 5"
        .byte     0xd,0xa
        .align  1
        jmp     TYPE
